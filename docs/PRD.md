...
### 5.16 模組：主動預算警報（LINE Push + Flex Message）

#### 5.16.1 觸發機制

**使用者故事**：
> 作為一個忙碌的家長，我希望在預算快要燒完或單日花得太兇時，系統能主動推 LINE 給我提醒，而不用我每天打開 App 自己查。

**功能規格**：
- `runBudgetAlerts(supabase, userId)` orchestrator 在每筆 expense INSERT / UPDATE / 核銷成功後被呼叫
- 接點：`actions/transactions.ts` createTransaction (expense) + updateTransaction；LINE webhook expense flow + tryConfirmPlaceholder
- **Scenario A — 月度活錢預警**：`monthlyRemaining / monthlyBudget < 20%` 命中
- **Scenario B — 單日熔斷**：`todaySpent >= dailyBaseline × 5` 命中
- 內部 try/catch 全包，警報失敗永不炸主流程

#### 5.16.2 去重狀態表（per 0016）

- `budget_alerts` 表 + `UNIQUE (user_id, alert_type, alert_period)`
- `alert_period` 對應：
  - `low_remaining` → `YYYY-MM`（同月只推一次）
  - `daily_burst` → `YYYY-MM-DD`（同日只推一次）
- INSERT 23505 conflict = 已推過、靜默 skip — PG 強原子保證、零 race condition

#### 5.16.3 LINE Flex Message 視覺

**Bubble 結構** (size=kilo, Apple 暗黑奢華 palette)：
```
┌─ #09090b (zinc-950) ─────────────────┐
│ 📡 MONEY RADAR 戰情預警        ●     │ ← header + 狀態燈
│  剩餘可用活錢                          │
│  $3,210                              │ ← hero 3xl 白字粗
...
**動態進度條**：
- 外殼 horizontal box (`#27272a` + 6px + rounded)
- 內條 `width: ${pct}%` + 顏色路由（< 20% red / >= 20% emerald）
- clamp [2%, 100%] 避免破版

**altText fallback**：LINE 不支援 Flex 的客戶端（舊版 / Apple Watch / 通知欄）顯示純文字摘要。

#### 5.16.4 三層 fallback 保護

| 缺什麼 | 行為 |
|---|---|
| `LINE_CHANNEL_ACCESS_TOKEN` 未設 | `runBudgetAlerts` 首行 return — fork repo 沒接 LINE 也跑得起來 |
| `NEXT_PUBLIC_SITE_URL` 未設 | footer 不渲染按鈕（只剩智囊文字） |
| LINE 客戶端不支援 Flex | 顯示 altText 純文字 |

---

### 5.17 模組：收入多維度分類（Salary / Side Hustle / Investment / Other）

#### 5.17.1 資料模型（per 0017）

**使用者故事**：
> 我希望系統能把我的收入拆成「主業薪資 / 副業外快 / 投資配息 / 其他流入」四類，這樣才能看出我的收入結構有沒有變多元化。

**功能規格**：
- `transactions.income_category TEXT CHECK IN ('salary','side_hustle','investment','other') OR NULL`
- 只有 `type='income'` 時有值；expense / transfer 強制 null
- 4 維度語意：
  - `salary` — 主業薪資 / 年終 / 獎金
...
#### 5.17.2 LINE LLM 自動分流

**LLM Prompt 規則 9**（per Phase 1 收尾）：
- 訊息含「薪水 / 薪資 / 工資 / 年終 / 獎金 / 發薪」→ `salary`
- 「接案 / 副業 / 外快 / 顧問費 / 兼差 / 講師費」→ `side_hustle`
- 「股息 / 配息 / 利息 / 中租 / 房租收入 / 股利」→ `investment`
- 「補助 / 退稅 / 退款 / 紅包 / 中獎 / 禮金」→ `other`
- 找不到明確線索的收入訊息 → `other`（保底，避免錯抓 expense category）

#### 5.17.3 首頁克制總覽

- BoardCard 在「本月已支出 / 剩餘額度」之間插一行 `text-xs text-zinc-500`：
  ```
  💰 本月已進帳總收入: $XXXXX
  ```
- 零收入時不渲染（避免空字串視覺垃圾）
- **設計哲學**：首頁是戰情室不該變財務報表，細節留給專業頁

#### 5.17.4 分析頁深度拆解 — 雙模式 CategoryPieCard + iPhone 級滑塊

**MonthCategoryCard 重構為支出 / 收入雙模式**：
- 頂部加自訂膠囊 segmented control：`[ 💸 支出 ] [ 💰 收入 ]`
- 滑塊背景用 `motion.div layoutId="activeModeIndicator"` — framer-motion 自動 morph 兩格之間 transform
- `transition: { type: 'spring', stiffness: 380, damping: 30 }` = iPhone 觸感校準值（0.2 秒完成 + 落點微吸附震動）
- 文字 `z-10` 浮在滑塊上方；選中純白 / 未選 zinc-500，`transition-colors duration-200` 跟滑塊同節奏
- 圖表切換用 `AnimatePresence mode="wait"`：0.2s fade + 8px Y 軸位移，內層 Recharts 500ms 扇區動畫接力

**Apple 暗黑 4 色 palette**：

| Category | Hex | Tailwind | 語意 |
|---|---|---|---|
| `salary` | `#3B82F6` | blue-500 | 深藍 — 穩定主業 |
| `side_hustle` | `#10B981` | emerald-500 | 翡翠綠 — 自由副業 |
| `investment` | `#F59E0B` | amber-500 | 琥珀黃 — 配息「金光」 |
| `other` | `#71717A` | zinc-500 | neutral |

#### 5.17.5 財務彈性 — 收入多元化智囊

- `buildFinancialElasticity` 計算 `IncomeBreakdown`（依 income_category 累加）
- `nonWagePct = (side_hustle + investment + other) ÷ totalIncome × 100`
- ≥ 10% 時 FinancialElasticity 元件顯示 emerald `DiversificationAlert`：
  ```
  💡 收入多元化表現優異
...
#### 5.17.6 財務彈性零收入 fallback（Bug Fix）

**問題**：原本月份初期 totalIncome=0 → burdenRate=NaN → tier 退化 safe → 誤判「彈性絕佳」。

**修法**：
- `buildFinancialElasticity` 多收 `recurring` 參數
...
| `budget_alerts` | LINE Push 預算警報去重狀態表（per 0016） |
...
| 收入分類 CHECK | `transactions.income_category IN ('salary','side_hustle','investment','other') OR NULL` |
| 警報去重 | `(user_id, alert_type, alert_period)` UNIQUE on `budget_alerts` |
...
- [x] 三維付款方式 + 現金錢包
- [x] **收入多維度分類**（salary / side_hustle / investment / other 4 維度）
- [x] 資產整合看板
- [x] 6 維度分析報表（含時光機）
- [x] 淨資產戰情室 + 月度快照
- [x] 夢想基金
...
- [x] **主動預算警報** — LINE Push + Flex Message 視覺圖卡（活錢預警 + 單日熔斷）
- [x] **財務彈性零收入 fallback** — 月初用 recurring 預期收入當分母，不再誤判「彈性絕佳」
- [x] **收入多元化智囊** — 非工資佔比 ≥ 10% emerald 鼓勵 alert
- [x] **雙模式 CategoryPieCard** — 支出 / 收入切換 + iPhone 級 spring 滑塊 (380/30)
- [x] 4-tier 防窺模式
...
| 0016 | budget_alerts 表 + UNIQUE 去重（月度警報 / 單日熔斷） |
| 0017 | transactions.income_category 多維度收入分類（4 enum 值） |
